...
const Login = () => {
  ...
  const loginHandler = async () => {
    ...
    try {
      let result = await axios.get(
        AppGlobal.apiBaseUrl + `User/${userObj.email}/${userObj.password}`
      );
      if (cookies.jwtToken) {
      ...
      if (!e.response.data.Email || !e.response.data.Password) {
        ...
        setErrorMsg({
          msg: (
            <div>
              <Alert severity="error">{e.response.data.Description}</Alert>
            </div>
          ),
        });
      }
  ...
  const googleLogin = async (values) => {
    if (values.googleId) {
      ...
      const password = values.googleId;
      const error = false;
     
      try {
        let result = await axios.get(
          AppGlobal.apiBaseUrl + `User/${email}/${password}`
        );
        if (cookies.jwtTokenGoogle) {
          removeCookie("jwtTokenGoogle");
        }
        setCookie("jwtTokenGoogle", JSON.stringify(result.data.token));
        history.push("/");
      } catch (e) {
          error = true;
        if (!e.response.data.Email || !e.response.data.Password) {
      ...
      if (error) {
        let formData = {
          firstName: values.profileObj.familyName,
          lastName: values.profileObj.givenName,
          email: values.profileObj.email,
          password: values.googleId,
          confirmPassword: values.googleId,
  ...
  const forgotPassword = () => {
      const data = {
          email: 'sammanmra008@gmail.com'
      }
      axios.post(`${AppGlobal.apiBaseUrl}/sendlink`, data)
      .then(res => alert("Check your mail, password reset link has been sent"))
      ...
      <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
        <div className={classes.paper}>
          ...
          <form className={classes.form} noValidate>
            ...
            <GoogleLogin
              clientId={AppGlobal.clientId}
              buttonText="Sign In With Google"
              onSuccess={(res) => googleLogin(res)}
              onFailure={(err) => console.log(err)}
              cookiePolicy={AppGlobal.cookiePolicy}
            />
            <Grid container>
              <Grid item xs>
